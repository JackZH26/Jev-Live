...
struct Axis
{
    ...
    double Step(double Current,double Target,double Dt,double MaxSpeed=170,double MaxAcceleration=850,double TargetVelocity=0)
    {
        ...
        TargetVelocity=std::isfinite(TargetVelocity)?std::clamp(TargetVelocity,-MaxSpeed,MaxSpeed):0;
        ...
        while(Remaining>0){const double H=std::min(Remaining,1.0/120.0);Remaining-=H;
            ...
            const double Acceleration=std::clamp(64.0*Error+16.0*(TargetVelocity-Velocity),-MaxAcceleration,MaxAcceleration);
