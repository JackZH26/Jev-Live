...
Private hosting rehearsals can explicitly project locally simulated comments into the X overlay. They must set the overlay's `simulation` flag and mark every injected message as simulated; the panel displays **SIMULATED CHAT** and viewer names carry **[SIM]**. These comments exercise voice replies locally. They are not sent to X or read from its actual audience. The regular X overlay continues to omit chat from other platforms. Default hosting is English; supported comment languages receive matching spoken replies, with English as the fallback.
