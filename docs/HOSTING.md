...
2026-09-21 更新：所有游戏解说与评论回复统一使用英语，恢复原版 Melo 英文少女声，暂停多语种语音和音色克隆。界面仍支持五语。见[自然主持改进方案](NATURAL_HOSTING.md)与[模拟评论演练](CHAT_REPLAY.md)；真实平台互动仍需单独验收。
...
3. 使用[内置 Live2D 少女](LIVE2D.md)，或导入有使用权的 PNG / WebP / JPEG / 自包含 VRM，单文件不超过 60 MB。导入文件复制到本机用户数据目录；不上传服务器或 Git。
4. 按 [本地模型说明](LOCAL_HOST.md) 启动 Ollama 和本机 Melo 语音服务，选择模型与 CPU/GPU，填写角色风格。默认使用「原版英文少女声 · 本地」；评论无论使用什么语言，回复和播报均为英语。旧 Qwen/克隆配置自动迁移，参考音频仍保留在本机。先试听，再启动主持。字幕与语音由 OBS Browser Source 合成；声音监控默认关闭，避免重复回声。
...
2026-09-21 的全英文播报与新少女验收见 [Live2D 与英文播报](LIVE2D.md)。下表保留旧版本记录。
