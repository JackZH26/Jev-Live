...
September 21 continuous-stream update: start the next match immediately from an available lobby or official result screen. Retry an unconsumed command on the same screen every 1.5 seconds; a fresh lobby does not inherit the former ten-second cooldown. Transition grace waits only for actual loading, allowing an actionable lobby to retry. Normal room selection remains active; loading time is measured separately from lobby dwell. The current native candidate's `new_match` goes straight into another match, adding no intermediate lobby dwell. Do not skip a result before it becomes authoritative; retain manual takeover, focus and connection protections.

Run `scripts/validate-hybrid-local.cjs --candidate <absolute candidate directory> --jev --live --continuous` for continuous operation without the former 30-minute/20-match/1200-request rehearsal limits. This requires explicit `--live` and does not qualify for strict streak acceptance. Create `stop-requested` inside the reported session directory to stop gracefully with release confirmation. Game exit, manual takeover or connection failure still stops control. Continuous mode retains rolling diagnostics and all official results; `transitions.ndjson` records phase timings for rematch verification.

This build is `59f195db79bb` (`8985f41`), packaged at `release/combat-20260921-r7/win-unpacked`, still paired with candidate `20260921-10`. All 214 tests, type checking, seven embedded strategy hashes and launcher version selection passed. A native start-epoch acknowledgment issue found in the real run was corrected: while acknowledgment is pending, retry only an offered start action without misclassifying takeover or rearming combat input. The continuous process reached normal room selection and gameplay, with the first lobby action issued in its first control cycle. End-to-next-match timing still requires an uninterrupted completed match; loading transitions and screenshots alone do not establish that acceptance.

The `7db03bed26ce` version and 30-minute rehearsal cap below describe the previous run. The build and continuous parameters above supersede them.
