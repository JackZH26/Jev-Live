...
app.whenReady().then(async()=>{
 ...
 let shown,standbyReady=false;
 async function prepareStandby(){if(standbyReady)return;const {sceneItems}=await obs.call('GetSceneItemList',{sceneName:'JEV Program'});let card=sceneItems.find(i=>i.sourceName==='JEV Test Status');const settings={is_local_file:true,local_file:path.join(__dirname,'standby.html'),width:1920,height:1080,fps:30,reroute_audio:true};if(!card){const r=await obs.call('CreateInput',{sceneName:'JEV Program',inputName:'JEV Test Status',inputKind:'browser_source',inputSettings:settings,sceneItemEnabled:true});card={sceneItemId:r.sceneItemId};}else await obs.call('SetInputSettings',{inputName:'JEV Test Status',inputSettings:settings,overlay:true});const game=sceneItems.find(i=>i.sourceName==='ETC Game');if(game)await obs.call('SetSceneItemIndex',{sceneName:'JEV Program',sceneItemId:card.sceneItemId,sceneItemIndex:Math.min(sceneItems.length-1,game.sceneItemIndex+1)});standbyReady=true;}
 async function showGameplay(active){if(!process.argv.includes('--bind-x-preview')||shown===active)return;await connectObs();await prepareStandby();const {sceneItems}=await obs.call('GetSceneItemList',{sceneName:'JEV Program'});for(const item of sceneItems){if(!['ETC Game','JEV Test Status'].includes(item.sourceName))continue;await obs.call('SetSceneItemEnabled',{sceneName:'JEV Program',sceneItemId:item.sceneItemId,sceneItemEnabled:item.sourceName==='ETC Game'?active:!active});}shown=active;}
 async function bindReview(meta,o){if(!process.argv.includes('--bind-x-preview')||boundPid===meta.pid||!o.foreground||Date.now()-o.timestamp>1000)return;
  ...
  await showGameplay(false);
  await obs.call('SetInputSettings',{inputName:'ETC Game',inputSettings:{window:''},overlay:true});
  await sleep(300);
 ...
 while(!stopped){try{
  ...
  if(!meta){await showGameplay(false);await sleep(2000);continue}
  ...
  await showGameplay(!receipt&&o.foreground&&Date.now()-o.timestamp<1500&&boundPid===meta.pid);
 ...
 }catch{await showGameplay(false).catch(()=>{});console.log('Console waiting for fresh readable telemetry or network.');}await sleep(2000);}
