...
export async function neuralSpeech(text:string,voice:string,language:string,signal:AbortSignal){
 ...
 const response=await fetch(endpoint+'/speech',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:text.slice(0,350),voice,language}),redirect:'error',signal:AbortSignal.any([signal,AbortSignal.timeout(35000)])});
