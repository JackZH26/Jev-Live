...
import {type VoiceReference,validReferenceText} from './voice-reference';
type NeuralProvider='qwen'|'qwen-clone'|'melo';
const endpoint=(provider:NeuralProvider)=>provider==='melo'?'http://127.0.0.1:11437':provider==='qwen-clone'?'http://127.0.0.1:11438':'http://127.0.0.1:11435';
export async function neuralSpeech(text:string,voice:string,language:string,signal:AbortSignal,provider:NeuralProvider='qwen',reference?:VoiceReference){
 signal.throwIfAborted();
 if(provider==='qwen-clone'&&(!reference?.audio||!validReferenceText(reference.text)))throw new Error('host.voiceReferenceRequired');
 const response=await fetch(endpoint(provider)+'/speech',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:text.slice(0,350),voice:provider==='qwen'?voice:'',language,...(provider==='qwen-clone'?{reference}:{} )}),redirect:'error',signal:AbortSignal.any([signal,AbortSignal.timeout(35000)])});
