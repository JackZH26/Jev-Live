...
export class Chat {
 readonly simulatedReplies:{id:string;platform:OAuthProvider;text:string;at:number}[]=[];
 ...
 constructor(private auth:OAuth,private receive:(m:ChatMessage)=>void,readonly mode:'live'|'simulation'='live'){this.platform=new Platforms(auth);}
 ingestReplay(m:ChatMessage){if(this.mode!=='simulation')throw new Error('Simulation input requires an isolated chat instance');if(!['youtube','twitch'].includes(m.platform)||!m.id||!m.authorId||!m.text.trim()||m.text.length>600||!Number.isFinite(m.at))throw new Error('Invalid replay message');this.add({...m,author:m.author.slice(0,80),simulation:true,self:false});}
 ...
 async start(providers:OAuthProvider[],youtubeId?:string){this.stop();this.controller=new AbortController();this.youtubeId='';if(this.mode==='simulation'){this.messages.splice(0);this.seen.clear();this.simulatedReplies.splice(0);for(const p of ['youtube','twitch'] as const)this.status[p]=blank();return;}for(const p of providers){this.status[p]=blank();this.status[p].state='connecting';if(p==='youtube')void this.youtube(youtubeId);else void this.twitch();}}
 ...
 async send(m:ChatMessage,text:string){if(this.mode==='simulation'){if(!m.simulation||!this.controller||this.controller.signal.aborted)throw new Error('host.chatOffline');this.simulatedReplies.push({id:m.id,platform:m.platform,text,at:Date.now()});if(this.simulatedReplies.length>200)this.simulatedReplies.shift();this.status[m.platform].sent++;return;}if(m.simulation)throw new Error('Replay messages must never be sent to a live platform');if(!this.controller||this.controller.signal.aborted||this.status[m.platform].state!=='connected')throw new Error('host.chatOffline');if(Date.now()-this.lastSend[m.platform]<5000)throw new Error('host.chatRate');this.lastSend[m.platform]=Date.now();const signal=AbortSignal.any([this.controller.signal,AbortSignal.timeout(15000)]);
