...
export class OverlayServer {
 ...
 async start(){this.server=createServer(async(req,res)=>{const fail=(code:number)=>{res.writeHead(code).end();};try{
  ...
  if(parts[1]==='view'&&providers.includes(p)&&req.method==='GET'){res.setHeader('Content-Type','text/html; charset=utf-8');res.setHeader('Content-Security-Policy',"default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; media-src 'self' blob:; connect-src 'self' blob:; worker-src 'self' blob:; frame-ancestors "+this.previewOrigin);res.end((await readFile(join(this.dist,'overlay.html'),'utf8')).replaceAll('./assets/','/assets/'));return;}
