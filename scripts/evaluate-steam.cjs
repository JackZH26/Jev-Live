...
(async()=>{
 ...
 const summary={label,at:new Date().toISOString(),out,results:[],interrupted:[],lobbyDelays:[],complete:false};let owned=false,current,previous,stopReason='budget',lastWrite=0,lastStatus=0,lobbyAt=0,lobbyEpoch=0,lobbyRearmedAt=0;
 try{
  ...
  while(Date.now()-started<minutes*60000){
   ...
   if(o.phase==='menu'&&!lobbyAt){lobbyAt=now;lobbyEpoch=o.epoch;lobbyRearmedAt=0;}
   if(o.phase==='menu'&&lobbyAt&&!lobbyRearmedAt&&o.mode==='auto'&&o.epoch>lobbyEpoch)lobbyRearmedAt=now;
   if(lobbyAt&&o.phase==='loading'){summary.lobbyDelays.push({at:new Date(now).toISOString(),seconds:(now-lobbyAt)/1000,afterResult:!!current?.finished,
    waitSeconds:lobbyRearmedAt?(lobbyRearmedAt-lobbyAt)/1000:null,transitionSeconds:lobbyRearmedAt?(now-lobbyRearmedAt)/1000:null});lobbyAt=0;}
   if(o.phase==='playing'&&current?.id!==o.matchId){
    ...
    current={id:o.matchId,startedAt:now,eligible:true,rooms:[],mechanicalSightings:0,mechanicalEngageFrames:0,hurtEvents:0,stalledMs:0,phase:o.phase,kills:0,damage:0,shots:0,playingSamples:0,nativeSamples:0,nativeDecisions:0,brainModes:{}};
   }
   ...
   if(current?.id===o.matchId&&!current.finished){
    ...
    current.kills=o.self.kills;current.damage=o.self.damageDealt??0;current.shots=Math.max(current.shots,o.diagnostics.shots??0);current.phase=o.phase;
