// Actual OBS encoders -> two loopback RTMP receivers. Never uses platform tokens.
...
(async()=>{
  ...
  const env={...process.env,JEV_TEST_DATA_DIR:path.join(process.env.LOCALAPPDATA,'JEV Studio DevTest')};delete env.ELECTRON_RUN_AS_NODE;delete env.JEV_DEV_URL;
  const receivers=[];let app;
  try {
    for(const [index,provider] of ['youtube','twitch'].entries()) {
    ...
    const result=await app.evaluate(async({safeStorage},{root,directory})=>{
      ...
      const {Obs}=require(require('node:path').join(root,'dist-main','electron','obs.js'));
      const store=new Store(directory,safeStorage);await store.init();const obs=new Obs(store,()=>{});await obs.setup();
      const names=['youtube','twitch'];
      ...
      try {
        for(const [i,p] of names.entries()) {
          ...
          await c.call('CreateInput',{sceneName:'JEV Program',inputName:'Local RTMP Acceptance',inputKind:kind,inputSettings:{width:1920,height:1080,color:i===0?0xff5555ee:0xffcc9944},sceneItemEnabled:true});
          await obs.service(p,`rtmp://127.0.0.1:${19351+i}/live`,'fixture');
        }
        await obs.start('youtube');await obs.start('twitch');await new Promise(r=>setTimeout(r,12000));await obs.poll();
    ...
    for(const receiver of receivers) {
      const streams=JSON.parse(execFileSync('ffprobe',['-v','error','-show_entries','stream=codec_type,codec_name,width,height,r_frame_rate','-of','json',receiver.file],{encoding:'utf8',windowsHide:true})).streams;
      ...
      if(!streams.some(s=>s.codec_type==='video'&&s.width===1920&&s.height===1080)||!streams.some(s=>s.codec_type==='audio')||bytes<50000)throw new Error(`Insufficient local output: ${receiver.provider}`);
    }
    if(JSON.stringify(report.receivers[0].pixel)===JSON.stringify(report.receivers[1].pixel))throw new Error('Outputs unexpectedly share a canvas');
