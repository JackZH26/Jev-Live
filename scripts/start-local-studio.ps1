param([string]$RuntimeDirectory='E:\JevRuntime')
$ErrorActionPreference='Stop'
$repositoryRoot=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..'))
$candidates=@((Join-Path $repositoryRoot 'release\hosting\win-unpacked\JEV Studio.exe'),(Join-Path $repositoryRoot 'release\win-unpacked\JEV Studio.exe'))
$executable=$candidates | Where-Object {Test-Path -LiteralPath $_} | Select-Object -First 1
if(-not $executable){throw 'Build JEV Studio first: npm run pack'}
