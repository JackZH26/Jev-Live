...
async function main(){
 ...
 await host.init();await host.save({...host.config,language:'en',compute:arg('compute')==='auto'?'auto':'cpu',speechProvider:'melo',chatPlatforms:[],commentary:false,textReplies:true,replyCooldownSec:5,maxPerHour:240,persona:'A relaxed gaming streamer. Reply naturally to the selected viewer in English regardless of their language, usually one short sentence. Do not greet on every reply. Do not pretend to have seen a historical chat event. Use at most 24 English words.'});
 ...
 await fs.writeFile(path.join(directory,'session.json'),JSON.stringify({url:address+base+'/',datasetPath,simulation:true,language:'en',replyLanguage:'Always English, regardless of comment language'},null,2));
