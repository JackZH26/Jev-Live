...
let stopRequested=false;for(const signal of ['SIGINT','SIGTERM'])process.on(signal,()=>{stopRequested=true;});
...
app.disableHardwareAcceleration();app.whenReady().then(async()=>{
 ...
 if(continuous&&!live)throw Error('continuous_requires_live');
 report.requestedMatches=continuous?null:requestedMatches;report.officialResults=[];report.liveDemonstration=live;report.continuous=continuous;
 ...
 const store=new Store(data,safeStorage);await store.init();const settings=settingsSchema.parse({decisionProvider:cloud?'jev':'rules',decisionIntervalMs:2500,autoRestart:continuous||requestedMatches>1});
 ...
 TypeSafeClient.prototype.systemOne=async function(...args){if(!continuous&&report.cloudCalls.length>=requestBudget)throw Error('request_budget');const row={at:Date.now(),state:structuredClone(args[0].state)};report.cloudCalls.push(row);if(continuous&&report.cloudCalls.length>200)report.cloudCalls.shift();try{const r=await call.apply(this,args);Object.assign(row,{ok:true,ms:Date.now()-row.at,model:r.model,action:r.answers.action.choice,destination:r.answers.destination?.choice});return r;}catch(e){Object.assign(row,{ok:false,ms:Date.now()-row.at,error:e.name});throw e;}};
 }
 ...
 let epoch=1;await control.change('auto',epoch);const started=Date.now();let sampleAt=0,lastFresh=Date.now(),savedSamples=0,sampleCount=0,stopCheckAt=0,lastStage='';
 while((continuous||Date.now()-started<seconds*1000)&&child.exitCode===null&&!stopRequested){
  if(continuous&&Date.now()-stopCheckAt>1000){stopCheckAt=Date.now();if(await fs.stat(path.join(out,'stop-requested')).then(()=>true,()=>false)){stopRequested=true;break;}}
  ...
  if(o){
   ...
   if(control.transitionUntil>0){
    ...
    if(o.phase==='playing'||o.phase==='menu'&&o.matchId!==control.transitionMatch)control.transitionUntil=0;
   }
   ...
   const stage=o.matchId+':'+o.phase;if(stage!==lastStage){lastStage=stage;await fs.appendFile(path.join(out,'transitions.ndjson'),JSON.stringify({at:Date.now(),matchId:o.matchId,phase:o.phase,frame:o.frame})+'\n');}
   await control.tick(settings,epoch);lastFresh=Date.now();
   if(continuous&&report.events.length>500)report.events.splice(0,report.events.length-500);
   ...
   if(Date.now()-sampleAt>500){sampleAt=Date.now();sampleCount++;report.samples.push({at:sampleAt,phase:o.phase,position:o.self.position,room:o.self.room,hp:o.self.health,magazine:o.self.magazine,shots:o.diagnostics.shots,action:o.diagnostics.lastAction,executor:o.executor,held:o.diagnostics.heldInputs,self:o.self,enemies:o.enemies,actions:o.actions,zone:o.zone,mapView:o.mapView,navigation:control.navigation.context(),knowledge:control.navigation.knowledge.context(o)});if(continuous&&report.samples.length>600)report.samples.shift();if(sampleCount%20===0)console.log(JSON.stringify({at:sampleAt,phase:o.phase,room:o.self.room,hp:o.self.health,mag:o.self.magazine,shots:o.diagnostics.shots,enemies:o.enemies.length,offers:o.actions.reduce((a,x)=>(a[x.kind]=(a[x.kind]??0)+1,a),{}),action:o.diagnostics.lastAction,executor:o.executor}));}
   if(control.summary.matches>report.officialResults.length){report.officialResult={...o.result,matchId:o.matchId,frame:o.frame,phase:o.phase};report.officialResults.push({...report.officialResult,at:Date.now(),kills:control.summary.kills,damageDealt:control.summary.damageDealt,shots:control.summary.shots});await fs.writeFile(path.join(out,'results.json'),JSON.stringify(report.officialResults,null,2));console.log(JSON.stringify({officialResult:report.officialResults.at(-1),matches:control.summary.matches}));
    if(!continuous&&control.summary.matches>=requestedMatches){report.stopReason=requestedMatches===1?'official_result':'demo_complete';break;}}
   if(sampleCount>=savedSamples+20){savedSamples=sampleCount;await fs.writeFile(path.join(out,'live.json'),JSON.stringify({pid:child.pid,candidate:report.candidate,continuous,summary:control.summary,cloudStats:control.stats,samples:report.samples.slice(-20),cloudCalls:report.cloudCalls.slice(-4)},null,2));}
   if(!continuous&&report.cloudCalls.length>=requestBudget){report.stopReason='request_budget';break;}
  }else if(Date.now()-lastFresh>15000){report.stopReason='no_fresh_state';break;}
  await sleep(40);
 }
 report.summary=control.summary;report.cloudStats=control.stats;report.stopReason??=stopRequested?'operator_stop':child.exitCode!==null?'game_exit':'time_budget';
 ...
 if(control){const requestedAt=Date.now(),releaseEpoch=Math.max(999,control.epoch+1);await control.change('manual',releaseEpoch).catch(()=>{});const {verifyManualRelease}=require('./verify-manual-release.cjs');report.releaseCheck=await verifyManualRelease({observe:pid=>control.observe(pid),pid:child?.pid,epoch:releaseEpoch,requestedAt});report.manualRelease=report.releaseCheck.confirmed;await control.close(releaseEpoch+1).catch(()=>{});}
